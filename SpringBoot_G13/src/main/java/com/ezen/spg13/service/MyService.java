...
public class MyService {
	...
	public int buy(String id, int amount, int error) {
		int result = 0;
		int n = 0;
		// 전달된 아이디와 구매갯수를  transaction1 , transaction2  두개의 테이블에  insert합니다
		// 하나이상의 데이터베이스 작업을 한단위로 묶어서  하나의 실행단위로 정의된 것을 트랜잭션이라고 합니다
		...
		try {
			...
			ptm.commit(status); // 영역 안의 모든 데이터베이스 작업의 실행적용 -  트랜잭션의 끝
			result = 1; 
		}catch( Exception e) {
			System.out.println("중간에 에러나서 둘다 실행 안됬어요");
